...
python translate_request_parallel_processor.py \
  --requests_filepath tmp.en \
  --save_filepath de.jsonl \
...
async def process_api_requests_from_file(
    ...
    seconds_to_sleep_each_loop = 3  # 1 ms limits max throughput to 1,000 requests per second
        ...
        while True:
            ...
            if next_request is None:
                if queue_of_requests_to_retry.empty() is False:
                    ...
                    try:
                        # get new request
                        request_json,triple = convert_sentence_into_request(next(requests),prompt_format)
    ...
    async def call_API(
        ...
        if error:
            ...
            if self.attempts_left:
                ...
                append_to_jsonl([self.triple, self.request_json, self.result, self.task_id], save_filepath)
                status_tracker.num_tasks_in_progress -= 1
                status_tracker.num_tasks_failed += 1
        else:
            #print(response)
            append_to_jsonl([self.triple, self.request_json, response, self.task_id], save_filepath)
            status_tracker.num_tasks_in_progress -= 1
...
def api_endpoint_from_url(request_url):
    ...
    return "/".join(request_url.split("/")[-2:])
...
def num_tokens_consumed_from_request(
    ...
    """Count the number of tokens in the request. Only supports chat completion requests."""
    encoding = tiktoken.get_encoding(token_encoding_name)
    # if completions request, tokens = prompt + n * max_tokens
    if api_endpoint == "chat/completions":
        messages = request_json["messages"]
        max_tokens = request_json.get("max_tokens", 5)
        n = request_json.get("n", 1)
        completion_tokens = n * max_tokens
        #print(messages)
        prompt_tokens = sum([len(encoding.encode(p["role"])) + len(encoding.encode(p["content"][0]["text"])) for p in messages])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--requests_filepath", default="./data/DeepSemantics_Questions.json")
    parser.add_argument("--save_filepath", default="./result/DeepSemantics_Prompt_1_GPT4V.json")
    parser.add_argument("--prompt_format", default="Prompt_1")
    parser.add_argument("--request_url", default="https://api.openai.com/v1/chat/completions")
    parser.add_argument("--api_key", default=None)
    parser.add_argument("--http_proxy", default="http://127.0.0.1:7890") 
    parser.add_argument("--max_requests_per_minute", type=int, default=3_000 * 0.5)
    parser.add_argument("--max_tokens_per_minute", type=int, default=10_000 * 0.5)
    ...
    if args.save_filepath is None:
        args.save_filepath = args.requests_filepath.replace(".jsonl", "results.jsonl")
